...
def create_hybrid_answer_func(rag_retriever: RAGRetriever):
    ...
    def hybrid_answer(question: str, param_db: dict, full_doc: str, mode: str = "local") -> str:
        ...
        # 2. RAG检索（语义理解能力）
        context = rag_retriever.retrieve(question, top_k=3)
        if context:
            prompt = f"""基于以下产品知识库内容回答问题。

            **重要规则**：
            1. 先判断知识库内容是否真的能回答用户问题
            2. 如果内容不相关或无法回答，直接说"知识库中没有相关信息"
            3. 不要编造答案
            ...
            return llm_router(prompt, mode=mode)

        # 3. 无检索结果：返回提示而非调用LLM（避免幻觉）
        return "抱歉，我的知识库中没有找到相关信息。请尝试询问产品相关的问题。"
